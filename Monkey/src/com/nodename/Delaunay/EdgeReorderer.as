package com.nodename.Delaunay {
	...
	internal final class EdgeReorderer {
		...
		private function reorderEdges(origEdges : Vector.<Edge>, criterion : Class) : Vector.<Edge> {
			...
			var firstPoint : ICoord = (criterion == Vertex) ? edge.leftVertex as ICoord : edge.leftSite as ICoord;
			var lastPoint : ICoord = (criterion == Vertex) ? edge.rightVertex as ICoord : edge.rightSite as ICoord;
			...
			while (nDone < n) {
				for (i = 1; i < n; ++i) {
					...
					edge = origEdges[i];
					var leftPoint : ICoord = (criterion == Vertex) ? edge.leftVertex as ICoord : edge.leftSite as ICoord;
					var rightPoint : ICoord = (criterion == Vertex) ? edge.rightVertex as ICoord : edge.rightSite as ICoord;
