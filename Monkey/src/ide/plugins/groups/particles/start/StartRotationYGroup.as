package ide.plugins.groups.particles.start {
	...
	public class StartRotationYGroup extends ParticleBaseGroup {
		...
		private function changeToRandomTwoConst(e : Event) : void {
			this.particle.startRotation[1] = new DataRandomTwoConst(0, 0);
			this.particle.build();
			this.app.dispatchEvent(new SelectionEvent(SelectionEvent.CHANGE));
		}
		
		private function changeToCurve(e : Event) : void {
			this.particle.startRotation[1] = new DataCurves(360);
			this.particle.build();
			this.app.dispatchEvent(new SelectionEvent(SelectionEvent.CHANGE));
		}
		
		private function changeToConst(e : Event) : void {
			this.particle.startRotation[1] = new DataConst(0);
			this.particle.build();
			this.app.dispatchEvent(new SelectionEvent(SelectionEvent.CHANGE));
		}
		
		private function changeRandomTwoConst(event:Event) : void {
			this.particle.startRotation[1] = new DataRandomTwoConst(minConst.value, maxConst.value);
			this.particle.build();
			this.app.dispatchEvent(new SelectionEvent(SelectionEvent.CHANGE));
		}
		...
		private function changeOne(event:Event) : void {
			this.particle.startRotation[1] = new DataConst(this.oneConst.value);	
			this.particle.build();
		}
		...
		override public function updateGroup(app:App, particle:ParticleSystem):void {
			...
			if (particle.startRotation[1] is DataConst) {
				this.orientation = HORIZONTAL;
				this.addControl(this.oneConst);
				this.oneConst.value = (particle.startRotation[1] as DataConst).value;
				this.minHeight = 20;
				this.maxHeight = 20;
			} else if (particle.startRotation[1] is DataRandomTwoConst) {
				this.orientation = HORIZONTAL;
				var randomTwoConst : DataRandomTwoConst = particle.startRotation[1] as DataRandomTwoConst;
				...
				this.minHeight = 20;
				this.maxHeight = 20;
			} else if (particle.startRotation[1] is DataCurves) {
				this.orientation = VERTICAL;
				var propCurves : DataCurves = particle.startRotation[1] as DataCurves;
