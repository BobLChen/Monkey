package L3D.loaders.parser {
	...
	import L3D.core.materials.filters.TextureMapFilter;
	import L3D.core.texture3d.Texture3D;
	import L3D.loaders.parser.utils.AssetType;
	...
	public class Max3DSParser extends EventDispatcher {
		...
		private function proceedParsing() : Boolean {
			...
			for (name in _unfinalized_objects) {
				var mesh : Mesh3D = constructObject(_unfinalized_objects[name]);
				if (mesh)
		...
		protected function finalizeAsset(mesh : Mesh3D, name : String = null) : void {
		...
		private function parseObjectAnimation(end : Number) : void {
			...
			if (name != '$$$DUMMY' && _unfinalized_objects.hasOwnProperty(name)) {
				...
				var mesh : Mesh3D = constructObject(vo, pivot);
		...
		private function constructObject(obj : ObjectVO, pivot : Vector3D = null) : Mesh3D {
			if (obj.type == AssetType.MESH) {
				...
				if (vo != null && vo.colorMap != null && vo.colorMap.url != null) {
					...
					if (isError == false) {
						shader = new Shader3D("shader", [new TextureMapFilter(new Texture3D(_parentDir + vo.colorMap.url))]);
					} else {
						shader = new Shader3D("shader", [new ColorFilter(vo.diffuseColor)]);
					}
				} else {
					shader = new Shader3D("shader", [new ColorFilter(defaultColor)]);
				}
				...
				for (i = 0; i < len; i++) {
					...
					surf.vertexVector.push(obj.verts[obj.indices[i] * 3 + 0], obj.verts[obj.indices[i] * 3 + 1], obj.verts[obj.indices[i] * 3 + 2], 0, 0, 0, obj.uvs[obj.indices[i] * 2 + 0], obj.uvs[obj.indices[i] * 2 + 1]);
					...
					if (nidx == 3) {
						...
						normal.normalize();
						var idx0 : int = surf.vertexVector.length / surf.sizePerVertex;
						surf.vertexVector[(idx0-3) * 8 + 3] = normal.x;
						surf.vertexVector[(idx0-3) * 8 + 4] = normal.y;
						surf.vertexVector[(idx0-3) * 8 + 5] = normal.z;
						surf.vertexVector[(idx0-2) * 8 + 3] = normal.x;
						surf.vertexVector[(idx0-2) * 8 + 4] = normal.y;
						surf.vertexVector[(idx0-2) * 8 + 5] = normal.z;
						surf.vertexVector[(idx0-1) * 8 + 3] = normal.x;
						surf.vertexVector[(idx0-1) * 8 + 4] = normal.y;
						surf.vertexVector[(idx0-1) * 8 + 5] = normal.z;
						nidx = 0;
					}
				}
