package L3D.loaders.parser.md5 {
	...
	import L3D.core.animator.SkeletonClipNode;
	import L3D.core.animator.pose.JointPose;
	import L3D.core.animator.pose.SkeletonPose;
	import L3D.core.base.Bounds3D;
	import L3D.utils.Cast;
	import L3D.core.base.Quaternion;

	/**
	 * todo:将解析过程放入脚本或者其他工具完成
	 * md5解析器
	 * @author Neil
	 */
	public class MD5AnimParser extends EventDispatcher {
		...
		/** numAnimatedComponents -> 每一帧的替换信息 */
		...
		public function get skeletonClipNode() : SkeletonClipNode {
			return _clip;
		}
		...
		private function translateClip() : void {
			...
			_clip = new SkeletonClipNode(1000 / _frameRate);
						
			for (var i : int = 0; i < _numFrames; ++i) {
				
				var skeletonPose : SkeletonPose = translatePose(_frameData[i]);
				
				if(_bounds.length > 0) {
					var bounds : Bounds3D = skeletonPose.bounds;
					skeletonPose.bounds.max = _bounds[i].max;
					skeletonPose.bounds.min = _bounds[i].min;
				...
				_clip.addGpuUsedFrame(skeletonPose);
