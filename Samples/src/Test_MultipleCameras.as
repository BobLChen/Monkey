package {
	...
	public class Test_MultipleCameras extends Sprite { 
		...
		public function Test_MultipleCameras() {
			super(); 
			...
			this.secondCamera = new Camera3D(new PerspectiveLens());
			this.secondCamera.clipScissor = true;
			this.secondCamera.viewPort = new Rectangle(0, 0, 400, 300);
			
			this.scene.addEventListener(Scene3D.CREATE_EVENT, onCreate);
			this.scene.addEventListener(Scene3D.POST_RENDER_EVENT, postRender);
			this.scene.addEventListener(Object3D.ENTER_FRAME_EVENT, update);
		}
		
		protected function update(event:Event) : void {
			this.secondCamera.transform.setPosition(0, 2, 0);			
			this.secondCamera.transform.rotateY(0.5);
		}
		
		protected function postRender(event:Event) : void {
			this.graphics.clear();
			this.graphics.lineStyle(1, 0xFF0000);
			this.graphics.drawRect(0, 0, 400, 300);
			
			this.scene.setupFrame(this.secondCamera);
			this.scene.context.clear(0, 0, 0, 1, 1, 0, Context3DClearMask.DEPTH);
			this.scene.render();
		}
		...
		protected function onCreate(event:Event) : void {
			...
			// 天空盒 sky.sky
			var sky : SkyboxLoader = new SkyboxLoader(this.res + "sky.sky");
			sky.load();
